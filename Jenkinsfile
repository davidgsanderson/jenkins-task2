...
// 2 slashes to comment
    stage('Deploy MYSQL App'){
      steps{
        sh 'docker run -d --name trio-mysql --network flask_app_network_2 --volume db-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${DATABASE_PASSWORD} -e MYSQL_DATABASE=flask-db trio-mysql'
      }
    }
    stage('Deploy Flask App'){
      steps{
        sh 'docker run -d --name flask-app --network flask_app_network_2 -e YOUR_NAME=Andrew -e MYSQL_ROOT_PASSWORD=${DATABASE_PASSWORD} flask-app'
      }
    }
